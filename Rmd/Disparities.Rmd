...
germ_data <- germline_patient_data %>% 
  ...
  select(Age_at_MMonly_diagnosis, Gender, raceeth, Race, Race1, Ethnicity, Ethnicity1, Whole, ISS, ISS_grp) %>%
...
<!-- Sequencing data helped us identify CH mutations in `r inline_text(ch_char, variable = CH_status, level = "CH", column = "MM patients at time of blood with WES")`.  -->
...
ch_demo <- germline_patient_data %>%
  select(Age_at_MMonly_diagnosis, 
         Gender, raceeth, Race1, Ethnicity1, ISS, ISS_grp, 
         CH_status) %>% 
...
germline_patient_data %>% 
  select(Age_at_MMonly_diagnosis,
         Gender, raceeth, Race, Ethnicity1, ISS, ISS_grp, CH_status,
...
germline_patient_data %>% 
  select(Age_at_MMonly_diagnosis,
         Gender, raceeth, Race1, Ethnicity1, ISS, ISS_grp, CH_status,
         time_to_hct, time_to_drug,
         Drugs_ever, HCT_ever) %>%
  tbl_summary(by = Race1, 
              sort = list(everything() ~ "frequency", ISS ~ "alphanumeric", ISS_grp ~ "alphanumeric"),
