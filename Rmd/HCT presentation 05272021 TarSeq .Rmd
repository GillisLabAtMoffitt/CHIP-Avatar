...
```
::: {.blue}
[HCT for Targeted Sequencing Patients]{style="color: white;"}
:::
...
# MM patients with Targeted Sequencing who received HCT
<br> 
<br>
...
hct_data %>%
  ...
  tbl_summary(by = Whole, label = list(days_blood_hct ~ "Time to HCT from Blood Collection (in days)", time_to_hct ~ "Time to HCT from MM diagnosis (in days)")) %>% 
  bold_labels() %>% as_gt() %>%  
...
Again, the first table include the name of the **FIRST regimen** name (The patient who had anti-DKK-1 before HCT is still here).
...
hct_data %>% 
  ...
  tbl_summary(by = Whole,
              label = list(first_regimen_name ~ "Name of first regimen (only) before HCT"),
              sort = list(everything() ~ "frequency")) %>% 
  bold_labels() %>% as_gt()  %>%
  gt::tab_style(style = gt::cell_text(color = "#0099CC"), locations = gt::cells_column_labels(everything()))
...
hct_data %>%
  ...
  gt::tab_style(style = gt::cell_text(color = "#0099CC"), locations = gt::cells_column_labels(everything()))
```
The last table doesn't count melphalan line.
<br> 
<br>
...
CH status is assessed by using the Avatar Targeted Sequencing data.
...
hct_data %>%
  select(Disease_Status_germline, CH_status_TS) %>% 
  tbl_summary(by = CH_status_TS, label = list(Disease_Status_germline ~ "Disease Status at Time of Blood Collection")) %>% 
  bold_labels() %>% as_gt() %>%
  gt::tab_style(style = gt::cell_text(color = "#0099CC"), locations = gt::cells_column_labels(everything()))
...
tbl1 <- hct_data %>% 
  ...
  mutate(Ethnicity = str_replace(Ethnicity, "Unknown", NA_character_)) %>% 
  select(Age_at_MMonly_diagnosis, 
         Gender, Race, Ethnicity, ISS, ISS_grp, CH_status_TS) %>%
  tbl_summary(by = CH_status_TS, label = list(Age_at_MMonly_diagnosis ~ "Age at MM diagnosis"),
              sort = list(everything() ~ "frequency", ISS ~ "alphanumeric", ISS_grp ~ "alphanumeric"),
              digits = list(c(Age_at_MMonly_diagnosis) ~ 2),
              missing = "no") %>% 
...
tbl2 <- hct_data %>%
  select(avatar_id, collectiondt_germline, starts_with("line_start"), CH_status_TS) %>% 
  ...
  select(line_bf_blood, CH_status_TS) %>% 
  tbl_summary(by = CH_status_TS,
...
hct_data %>%
  distinct(avatar_id, .keep_all = TRUE) %>% 
  select(CH_status_TS, MM_age_cat) %>% 
...
```{r}
# hct_data %>%
#   select(avatar_id, starts_with("treatment_line_"), CH_status_TS) %>% 
#   pivot_longer(cols = starts_with("treatment_line_"),
#                names_to = "regimen_number", values_to = "treatment_line_", values_drop_na = TRUE) %>% 
#   filter(treatment_line_ < 90) %>% 
#   arrange(desc(treatment_line_)) %>% 
...
#     TRUE                                    ~ as.character(treatment_line_)
#   )) %>% 
#   select(treatment_line_, CH_status_TS) %>% 
...
# 
# hct_data %>%
#   select(avatar_id, date_of_bmt_1, starts_with("line_start"), CH_status_TS) %>% 
#   pivot_longer(cols = starts_with("line_start"),
#                names_to = "line", values_to = "date", values_drop_na = TRUE) %>% 
...
#   ungroup() %>% 
#   distinct(avatar_id, .keep_all = TRUE) %>% 
#   select(line_bf_hct, CH_status_TS) %>% 
...
#### In Full Targeted Sequencing Population
...
myplot <- survfit(mysurv~CH_status_TS, data = hct_patient_surv)
...
myplot <- survfit(mysurv~CH_status_TS, data = no_hct_patient_surv)
# jpeg(paste0(path, "/Figures/Survivals/CHIP/OS HCT by CH.jpeg"), width = 1200, height = 900)
...
::: {.col-md-6}
```{r, fig.height = 7}
tbl1 <- hct_patient_surv %>% select(CH_status_TS) %>% 
  tbl_uvregression(method = survival::coxph, 
...
tbl2 <- coxph(Surv(time = hct_patient_surv$month_at_os, 
             event = hct_patient_surv$os_event) ~ CH_status_TS, data =  hct_patient_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariate**", "**Multivariate**"))
```
<br><br><br>
```{r, fig.height = 7}
tbl1 <- no_hct_patient_surv %>% select(CH_status_TS) %>% 
  tbl_uvregression(method = survival::coxph, 
...
tbl2 <- coxph(Surv(time = no_hct_patient_surv$month_at_os, 
             event = no_hct_patient_surv$os_event) ~ CH_status_TS, data =  no_hct_patient_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariate**", "**Multivariate**"))
```
:::
...
tbl2 <- coxph(Surv(time = hct_patient_surv$month_at_progression_drug, 
             event = hct_patient_surv$hct_progression_event) ~ CH_status_TS, data =  hct_patient_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariate**", "**Multivariate**"))
```
<br><br><br>
```{r, fig.height = 7}
tbl1 <- no_hct_patient_surv %>% select(CH_status_TS, Age_at_MMonly_diagnosis, Gender, Ethnicity1, Race1, ISS) %>% 
  tbl_uvregression(method = survival::coxph, 
...
tbl2 <- coxph(Surv(time = no_hct_patient_surv$month_at_progression_drug, 
             event = no_hct_patient_surv$hct_progression_event) ~ CH_status_TS + Age_at_MMonly_diagnosis + Gender + Ethnicity1 + Race1 + ISS, data =  no_hct_patient_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariate**", "**Multivariate**"))
```
:::
...
#### In Targeted Sequencing Patient with Blood Collection Before HCT
...
myplot <- survfit(mysurv~CH_status_TS, data = blood_bf_hct)
...
::: {.col-md-6}
```{r, fig.height = 7}
blood_bf_hct %>% select(CH_status_TS) %>% 
  tbl_uvregression(method = survival::coxph, 
...
::: {.col-md-6}
```{r, fig.height = 7}
tbl1 <- blood_bf_hct %>% select(CH_status_TS) %>%
  tbl_uvregression(method = survival::coxph,
...
tbl2 <- coxph(Surv(time = blood_bf_hct$month_at_progression_drug,
             event = blood_bf_hct$hct_progression_event) ~ CH_status_TS, data =  blood_bf_hct) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariate**", "**Multivariate**"))
```
:::
