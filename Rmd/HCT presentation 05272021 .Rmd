...
Also `r emo::ji("question_mark")` to do PFS, we need the time point "zero" for the "No HCT" population.  
...
myplot <- survfit(mysurv~HCT_ever+CH_status, data = hct_patient_surv)
...
hct_patient_surv <- germline_patient_data %>% 
  filter(is.na(date_of_bmt_1)) %>% 
  distinct(avatar_id, .keep_all = TRUE)

no_hct_patient_surv <- germline_patient_data %>% distinct(avatar_id, .keep_all = TRUE)
mysurv <- Surv(time = no_hct_patient_surv$month_at_os, event = no_hct_patient_surv$os_event)
myplot <- survfit(mysurv~HCT_ever+CH_status, data = no_hct_patient_surv)
# jpeg(paste0(path, "/Figures/Survivals/CHIP/OS HCT by CH.jpeg"), width = 1200, height = 900)
...
::: {.col-md-6}
```{r, fig.height = 7}
tbl1 <- hct_patient_surv %>% select(HCT_ever, CH_status) %>% 
  tbl_uvregression(method = survival::coxph, 
...
tbl2 <- coxph(Surv(time = hct_patient_surv$month_at_os, 
             event = hct_patient_surv$os_event) ~ HCT_ever + CH_status, data =  hct_patient_surv) %>%
...
tbl2 <- coxph(Surv(time = hct_patient_surv$month_at_progression_drug, 
             event = hct_patient_surv$hct_progression_event) ~ HCT_ever + CH_status, data =  hct_patient_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariate**", "**Multivariate**"))
```
:::
