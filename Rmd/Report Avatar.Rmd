---
title: "Gillis Lab (We know about CHIP!)"
...
```{r, echo=FALSE}
htmltools::img(src = knitr::image_uri("/Users/colinccm/Documents/GitHub/CHIP-Avatar/chips.jpg"), 
               alt = 'logo', 
               style = 'position:absolute; top:0; right:0; padding:10px; width: 250px; height: 128px')
```
...
<span style="color: white;">Report on year 1 Avatar data Multiple Myeloma patients</span>
...
The disease status is evaluated by following these criteria:


**Pre Treatment Newly Diagnosed Multiple Myeloma**: Newly diagnosed multiple myeloma biopsy <span style="text-decoration:underline">before treatment initiation or within 1 month</span> of MM treatment initiation  
**Post Treatment Newly Diagnosed Multiple Myeloma**: Patients who have received treatment for MM but <span style="text-decoration:underline"> have not progressed</span> by the time of biopsy  
**Early Relapse Multiple Myeloma**: Patients who have received <span style="text-decoration:underline">1-3 lines of therapy</span> by the time of biopsy  
**Late Relapse Multiple Myeloma**: Patients who have received <span style="text-decoration:underline">> 3 lines of therapy</span> by the time of biopsy  
 
*Line of therapy = Group of drugs from treatment initiation to progression/relapse
...
I left the PFS from Dx for the disease status who are not treated with drugs.
Amyloidosis|MYELOFIBROSIS|Solitary|WALDENSTROM|Refractory|Normal|Poly are removed.
<div class = "row">
<div class = "col-md-6">
```{r, fig.height = 7}
...
***
## 2.Treatments
The results presented here are for the treatment of the germline patients population.
```{r}
...
```{r}
Treatment1 %>%
  mutate(Whole = "All regimen in ALL regimen for ALL patients!!!!") %>% 
  # distinct(avatar_id, .keep_all = TRUE) %>% 
  select(regimen_name, Whole) %>% 
  tbl_summary(by = Whole,
              sort = list(everything() ~ "frequency")) %>% bold_labels() %>%  as_gt() %>%  
  gt::tab_options(column_labels.border.bottom.color = "red",
...
germline_patient_data %>%
  ...
  tbl_summary(by = Disease_Status_germline,
              sort = list(everything() ~ "frequency"),
              missing_text = "No Drugs") %>% add_p() %>% add_n() %>% as_gt()
```
<br>
<br>
<!-- Leave a space here -->
...
myplot <- survfit(mysurv~pfs_hct, data = germline_patient_surv)
ggsurvplot(myplot, data = germline_patient_surv,
           title = "PFS HCT from  drug date (pfs_hct is when HCT <or= germline date)",
           font.main = c(24, "bold", "black"),
           font.x = c(20, "bold", "black"),
           font.y = c(20, "bold", "black"),
           font.legend = c(20, "bold", "black"),
...
myplot <- survfit(mysurv~pfs_hct+ISS, data = germline_patient_surv)
ggsurvplot(myplot, data = germline_patient_surv,
           title = "PFS HCT from hct (pfs_hct is when HCT <or= germline date)",
           font.main = c(24, "bold", "black"),
           font.x = c(20, "bold", "black"),
           font.y = c(20, "bold", "black"),
           font.legend = c(20, "bold", "black"),
...
myplot <- survfit(mysurv~pfs_radiation, data = germline_patient_surv)
ggsurvplot(myplot, data = germline_patient_surv,
           title = "PFS Radiation from rad (pfs_radiation is when rad <or= germline date)",
           font.main = c(24, "bold", "black"),
           font.x = c(20, "bold", "black"),
           font.y = c(20, "bold", "black"),
           font.legend = c(20, "bold", "black"),
...
knitr::include_graphics('/Users/colinccm/Documents/GitHub/CHIP-Avatar/Rplot.png')
```
