...
# Focus on Germline data and active MM patients
# I.Disparities
...
germline_patient_data %>% 
  ...
  tbl_summary(by = Whole, 
              ...
              digits = list(c(Age_at_diagnosis_closest_germline, Age_at_MMonly_diagnosis) ~ 2)) %>% bold_labels() %>%  as_gt() %>%  
  gt::tab_source_note(gt::md("*ISS is reported for active MM patients only*"))
```

## 2. Hypo : **Hispanic** population shows a lower age at diagnosis and worse PFS 
...
#   labs(x=NULL, y="Age at Diagnosis", title="Age of diagnosis closest to germline repartition")
# p + geom_jitter(shape=16, position=position_jitter(0.2)) +
...
germline_patient_data %>% 
  ...
  select(Age_at_diagnosis_closest_germline, Age_at_MMonly_diagnosis, diagnosis_MM_year,
         delay_to_treatment, Drugs_ever, HCT_ever, Gender, Race1, Ethnicity, ISS, CH_status) %>%
...
I couldn't add Drugs_ever in the coxph.
<div class = "row">
<div class = "col-md-6">
```{r, fig.height = 7}
...
```{r}
mysurv <- Surv(time = ethnicity_surv$month_at_progression_drug, event = ethnicity_surv$drug_progression_event)
myplot <- survfit(mysurv~Ethnicity+diagnosis_MM_year, data = ethnicity_surv)
ggsurvplot_facet(myplot, data = ethnicity_surv, facet.by = "diagnosis_MM_year",
           title = "PFS by Ethnicity from drug",
           font.main = c(24, "bold", "black"),
           font.x = c(20, "bold", "black"),
           font.y = c(20, "bold", "black"),
...
### CH not involved (wasn't different between Ethnicity)

```{r, fig.height = 7}
mysurv <- Surv(time = ethnicity_surv$month_at_progression_drug, event = ethnicity_surv$drug_progression_event)
myplot <- survfit(mysurv~Ethnicity + CH_status, data = ethnicity_surv)
ggsurvplot(myplot, data = ethnicity_surv,
           title = "PFS by Ethnicity/CH from drug",
           font.main = c(24, "bold", "black"),
           font.x = c(20, "bold", "black"),
           font.y = c(20, "bold", "black"),
           font.legend = c(20, "bold", "black"),
...
## 3. Hypo : **Black** population shows a lower age at diagnosis and worse OS 
...
p <- germline_patient_data %>% 
  mutate(Race = factor(Race, levels=c("White", "Black"))) %>% filter(!is.na(Race), !is.na(Disease_Status_facet)) %>%   ggplot(aes(x=Race1, y=Age_at_MMonly_diagnosis), fill=Race1) + 
...
#   ggplot(aes(x=Race1, y=Age_at_diagnosis_closest_germline), fill=Race1) + 
...
### Summary
Need to restrict to MM only patients
```{r demo in germline}
germline_patient_data %>% 
  mutate(Whole = "Germline patients") %>% 
  select(Age_at_diagnosis_closest_germline, Age_at_MMonly_diagnosis, diagnosis_MM_year, MM_age_cat3, MM_age_cat5,
         delay_to_treatment, Drugs_ever, HCT_ever, Gender, Race1, Ethnicity, ISS, CH_status) %>%
...
<div class = "col-md-6">
```{r, fig.height = 7}
race_surv <- germline_patient_surv %>% filter(!is.na(Race))
...
```{r, fig.height = 7}
mysurv <- Surv(time = race_surv$month_at_os, event = race_surv$os_event)
myplot <- survfit(mysurv~CH_status, data = race_surv)
ggsurvplot(myplot, data = race_surv, facet.by = "MM_age_cat",
           title = "OS by CH",
           font.main = c(24, "bold", "black"),
           font.x = c(20, "bold", "black"),
           font.y = c(20, "bold", "black"),
...
mysurv <- Surv(time = race_surv$month_at_os, event = race_surv$os_event)
myplot <- survfit(mysurv~CH_status, data = race_surv)
ggsurvplot(myplot, data = race_surv, facet.by = "MM_age_cat5",
           title = "OS by CH",
           font.main = c(24, "bold", "black"),
           font.x = c(20, "bold", "black"),
           font.y = c(20, "bold", "black"),
...
mysurv <- Surv(time = race_surv$month_at_os, event = race_surv$os_event)
myplot <- survfit(mysurv~CH_status, data = race_surv)
ggsurvplot(myplot, data = race_surv, facet.by = "MM_age_cat3",
           title = "OS by CH",
           font.main = c(24, "bold", "black"),
           font.x = c(20, "bold", "black"),
           font.y = c(20, "bold", "black"),
...
race_black <- race_surv %>% filter(Race1 == "Black")
...
tbl2 <- coxph(Surv(time = race_black$month_at_os, 
                   event = race_black$os_event) ~ Gender + ISS + HCT_ever + CH_status, data =  race_black) %>%
  tbl_regression(exponentiate = TRUE,
                 label = list(CH_status ~ "CH status",
                              HCT_ever ~ "HCT ever")
                 ) %>% bold_p(t = .05)
tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariate**", "**Multivariate**")) %>% as_gt() %>% 
...
ggsurvplot(myplot, data = race_black, facet.by = "MM_age_cat",
           title = "OS by CH",
           font.main = c(24, "bold", "black"),
           font.x = c(20, "bold", "black"),
           font.y = c(20, "bold", "black"),
...
germline_patient_data %>%
  ...
  tbl_summary(by = Disease_Status_facet, 
              sort = list(everything() ~ "frequency", ISS ~ "alphanumeric"),
              digits = list(c(Age_at_diagnosis_closest_germline, Race) ~ 2)) %>% add_p() %>% bold_p(t = .05) %>% add_overall() %>% bold_labels() %>%  as_gt() %>%  
  gt::tab_source_note(gt::md("*ISS is reported for active MM patients only*"))

race_surv %>%
  select(Race1, Disease_Status_facet) %>% 
  tbl_summary(by = Race1, 
              sort = list(everything() ~ "frequency")) %>% add_p() %>% bold_p(t = .05) %>% add_overall() %>% bold_labels() %>%  as_gt()

germline_patient_data %>%
  select(Ethnicity, Disease_Status_facet) %>% 
  tbl_summary(by = Ethnicity, 
              sort = list(everything() ~ "frequency")) %>% add_p() %>% bold_p(t = .05) %>% add_overall() %>% bold_labels() %>%  as_gt()
...
germline_patient_data %>%
  ...
  tbl_summary(by = CH_status, 
              sort = list(everything() ~ "frequency", ISS ~ "alphanumeric"),
              digits = list(c(Age_at_diagnosis_closest_germline, Race) ~ 2)) %>% add_p() %>% bold_p(t = .05) %>% bold_labels() %>%  as_gt() %>%  
...
```{r}
germline_patient_data %>%
  select(CH_status, HCT_ever, HCT_before_germline, HCT_vs_germline, 
         radiation_before_germline, Radiation_ever) %>% 
  tbl_summary(by = CH_status, 
              sort = list(everything() ~ "frequency")) %>% 
  add_p() %>% bold_p(t = .05) %>% bold_labels() %>% add_overall() %>% as_gt()

germline_patient_data %>%
  select(CH_status, HCT_before_germline) %>% 
  tbl_summary(by = HCT_before_germline, 
              sort = list(everything() ~ "frequency")) %>% 
  add_p() %>% bold_p(t = .05) %>% bold_labels() %>% add_overall() %>% as_gt()

germline_patient_data %>%
  select(CH_status, HCT_vs_germline) %>% 
...
```{r, fig.height = 7}
mysurv <- Surv(time = germline_patient_surv$month_at_os, event = germline_patient_surv$os_event)
myplot <- survfit(mysurv~drugs_before_germline, data = germline_patient_surv)
# jpeg(paste0(path, "/Figures/Survivals/Treatment/OS Drugs.jpeg"), width = 1200, height = 900)
ggsurvplot(myplot, data = germline_patient_surv,
           title = "OS Drugs (drugs_before_germline is when drugs <or= germline date)",
           font.main = c(24, "bold", "black"),
           font.x = c(20, "bold", "black"),
           font.y = c(20, "bold", "black"),
           font.legend = c(20, "bold", "black"),
...
ggsurvplot(myplot, data = germline_patient_surv,
           title = "PFS Drugs from drug date (drugs_before_germline is when drugs <or= germline date)",
           font.main = c(24, "bold", "black"),
           font.x = c(20, "bold", "black"),
           font.y = c(20, "bold", "black"),
           font.legend = c(20, "bold", "black"),
