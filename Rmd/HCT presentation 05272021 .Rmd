...
hct_data %>%
  mutate(Whole = "HCT patients") %>% 
  select(days_blood_hct, Whole, time_to_hct) %>% 
  tbl_summary(by = Whole, label = list(days_blood_hct ~ "Time from Blood Collection to HCT (in days)", time_to_hct ~ "Time from MM diagnosis to HCT (in days)")) %>% 
  bold_labels() %>% as_gt() %>%  
  gt::tab_source_note(gt::md("**HCT happen 690 days before blood collection**")) %>%
  gt::tab_style(style = gt::cell_text(color = "#0099CC"), locations = gt::cells_column_labels(everything()))

p <- qplot(x =days_blood_hct, data=subset(hct_data,!is.na(days_blood_hct)), fill=..count.., 
           geom="histogram", 
           binwidth = 100,
           ) 
...
hct_data %>% 
  mutate(days_blood_hct = (interval(start= hct_data$collectiondt_germline, end= hct_data$date_of_bmt_1)  /  duration(n=1, unit="days"))) %>% 
  mutate(HCT_vs_germline = str_replace(HCT_vs_germline, "No HCT", NA_character_)) %>% 
  select(HCT_vs_germline, days_blood_hct, HCT_vs_germline) %>% 
  tbl_summary(by = HCT_vs_germline, label = list(days_blood_hct ~ "Time to HCT from Blood Collection (in days)"),
              sort = list(everything() ~ "frequency"),
              missing = "no") %>% 
  bold_labels() %>% as_gt() %>%  
  gt::tab_style(style = gt::cell_text(color = "#0099CC"), locations = gt::cells_column_labels(everything()))
...
germline_patient_data %>% 
  ...
  tbl_summary(by = CH_status, 
              ...
              sort = list(everything() ~ "frequency"),
              missing = "no") %>% 
  bold_labels() %>% add_overall() %>% add_p(correct = FALSE) %>% bold_p(t = .05) %>% as_gt() %>%  
...
germline_patient_data %>% 
  ...
  annotate("text", x = 0, y = 105, label = paste0("p=",chisq.test(table(germline_patient_data$CH_status, germline_patient_data$HCT_ever), correct = FALSE)[3]),
...
hct_data %>% 
  ...
  geom_bar(stat = "identity")+
  labs(x = "", y = "percent", title = "Patients who had HCT",
       fill = "CH status") +
  ...
  geom_text(aes(label = paste0("n=", count)), size = 5, position = position_stack(vjust = 0.25))+
  annotate("text", x = 0, y = 105, label = paste0("p=",chisq.test(table(hct_data$CH_status, hct_data$HCT_vs_germline), correct = FALSE)[3]),
...
analysis_data %>% 
  ...
  annotate("text", x = 0, y = 105, label = paste0("p=",chisq.test(table(analysis_data$CH_status, analysis_data$HCT_vs_germline))[3]),
           color = "black", size = 6, hjust = 0, vjust = 0)
```
It is bias because CH patients are probably older and will not receive HCT anyway. Can we adjust?
