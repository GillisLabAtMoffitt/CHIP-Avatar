...
drugs_data %>%
  ...
  tbl_summary(by = CH_status,
              ...
              missing = "no") %>%
  bold_labels() %>% add_p(all_categorical() ~ "chisq.test") %>% bold_p(t = .05) %>% as_gt() %>%
  gt::tab_style(style = gt::cell_text(color = "#0099CC"), locations = gt::cells_column_labels(everything()))
...
drugs_data %>%
  ...
  annotate("text", x = 0, y = 105, label = paste0("p=",chisq.test(table(drugs_data$drugs_before_germline, drugs_data$CH_status))[3]),
...
::: {.col-md-6}
```{r, fig.height = 7}
tbl1 <- blood_bf_Drugs %>% select(CH_status, Age_at_MMonly_diagnosis, Gender, Ethnicity1, Race1, ISS) %>% 
  tbl_uvregression(method = survival::coxph, 
...
tbl2 <-
  coxph(Surv(time = blood_bf_Drugs$month_at_os,
             event = blood_bf_Drugs$os_event) ~ CH_status + Age_at_MMonly_diagnosis + Gender + Ethnicity1 + Race1 + ISS, data =  blood_bf_Drugs) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariate**", "**Multivariate**"))
```
:::
...
tbl2 <- coxph(Surv(time = blood_bf_Drugs$month_at_progression_drug,
             event = blood_bf_Drugs$drug_progression_event) ~ CH_status + Age_at_MMonly_diagnosis + Gender + Ethnicity1 + Race1 + ISS, data =  blood_bf_Drugs) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariate**", "**Multivariate**"))
```
:::
...
::: {.col-md-6}
```{r}
tbl1 <- drugs_data %>% select(regimen_category_1, Age_at_MMonly_diagnosis, Gender, Ethnicity1, Race1) %>% 
  tbl_uvregression(method = survival::coxph, 
...
tbl2 <-
  coxph(Surv(time = drugs_data$month_at_os,
             event = drugs_data$os_event) ~ regimen_category_1 + Age_at_MMonly_diagnosis + Gender + Ethnicity1 + Race1, data =  drugs_data) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariate**", "**Multivariate**"))
```
:::
...
tbl2 <- coxph(Surv(time = drugs_data$month_at_progression_drug,
             event = drugs_data$drug_progression_event) ~ regimen_category_1 + Age_at_MMonly_diagnosis + Gender + Ethnicity1 + Race1, data =  drugs_data) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariate**", "**Multivariate**"))
```
:::
