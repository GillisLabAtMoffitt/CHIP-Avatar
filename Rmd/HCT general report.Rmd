...
hct_data %>%
  ...
  tbl_summary(by = Whole, label = list(days_at_hct_blood ~ "Time to HCT from Bood Collection (in days)", time_to_hct ~ "Time to HCT from MM diagnosis (in days)")) %>% 
  bold_labels() %>% as_gt() %>%  
...
p + scale_fill_viridis_c(
  ...
  labs(x="Time to HCT from Bood Collection (in days)", y="Number of Patient", title="Number of Days Between Bood Collection and HCT",
       caption = "1 bin = 100 days")+
...
hct_data %>% 
  ...
  tbl_summary(by = HCT_vs_germline, label = list(days_at_hct_blood ~ "Time to HCT from Bood Collection (in days)"),
              sort = list(everything() ~ "frequency"),
              missing = "no") %>% 
  bold_labels() %>% as_gt() %>%  
  # gt::tab_source_note(gt::md("*ISS is reported for MM patients only*")) %>%  
...
Patients who had their blood collection before treatment were selected for sequencing.
...
### Disease status at time of bood collection (`r emo::ji("warning")` is not at time of HCT)
```{r}
hct_data_sequenced %>%
  mutate(Whole = "HCT patients") %>% 
  select(Disease_Status_germline, Whole) %>% 
