...
germline_patient_data <- germline_patient_data %>% filter(!is.na(date_of_MMonly_diagnosis)) %>% 
...
ch_demo <- germline_patient_data %>%
  ...
  select(Age_at_MMonly_diagnosis, 
         Gender, Race1, Ethnicity1, ISS, ISS_grp, 
         CH_status) %>% 
...
Within the MM Avatar data, `r length(unique(germline_patient_data$avatar_id))` patients had blood sequenced and were analyzed for CH mutations and CH was identified in `r inline_text(ch_samples, variable = CH_status, level = "CH")` patients. Age  shows a  difference between patient with CH `r inline_text(ch_demo, variable = Age_at_MMonly_diagnosis, column = "CH")`
Patients exhibiting CH shows a significant higher age at diagnosis compare to no CH individuals `r inline_text(ch_demo, variable = Age_at_MMonly_diagnosis, column = "CH")` and `r inline_text(ch_demo, variable = Age_at_MMonly_diagnosis, column = "No CH")` (`r inline_text(ch_demo, variable = Age_at_MMonly_diagnosis, column = "p.value")`).  
No differences werw observed in Gender, Race, Ethnicity but a trend toward a higher ISS clinical stage was noted (`r inline_text(ch_demo, variable = ISS_grp, level = "II-III", column = "CH")` compare to `r inline_text(ch_demo, variable = ISS_grp, level = "II-III", column = "No CH")`, `r inline_text(ch_demo, variable = ISS_grp, column = "p.value")`).
...
germ_data <- germline_patient_data %>% 
  ...
  select(Age_at_MMonly_diagnosis, Gender, Race, Race1, Ethnicity, Ethnicity1, Whole, ISS, ISS_grp) %>%
...
Sequencing data helped us identify CH mutations in `r inline_text(ch_char, variable = CH_status, level = "CH", column = "MM patients with WES")`. + Add bioinfo results.

## 3. Disease Status

These patients are now all MM but at time of blood collection distribution of disease status is as below. (would probably keep that for a treatment paper)
```{r}
...
Interestingly, we noticed a significant difference between disparities on the age at diagnosis. Hispanic are diagnosed at an earlier age `r inline_text(eth_age, variable = Age_at_MMonly_diagnosis, column = "Hispanic", pattern = "{median} years old")` compare to `r inline_text(eth_age, variable = Age_at_MMonly_diagnosis, column = "Non-Hispanic", pattern = "{median} years old")` (`r inline_text(eth_age, variable = Age_at_MMonly_diagnosis, column = "p.value")`). Blacks also exhibit a lower age at diagnosis compare to Whites, respectively `r inline_text(race_age, variable = Age_at_MMonly_diagnosis, column = "Black", pattern = "{median} years old")` vs `r inline_text(race_age, variable = Age_at_MMonly_diagnosis, column = "White", pattern = "{median} years old")` (`r inline_text(race_age, variable = Age_at_MMonly_diagnosis, column = "p.value")`).
...
germline_patient_data %>% 
  distinct(avatar_id, .keep_all = TRUE) %>% 
  select(Age_at_MMonly_diagnosis,
         Gender, Race1, Ethnicity1, ISS, ISS_grp, CH_status,
...
No difference in OS within Ethnicity. The hazard ratio is decreased by 26% for Hispanic population but is not significant. Notably, age at diagnosis, ISS (ISS_grp) and receiving HCT are factor involved in OS in univariate and multivariate analysis. (Give data)
...
We noted that in multivariate analysis being a male increase significantly the HR of progression by 29% (add data, pval = 0.029). 
...
eth_cox_os <- tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariate**", "**Multivariate**"))
eth_cox_os
```
</div>
...
tbl2 <- coxph(Surv(time = ethnicity_surv$month_at_progression_drug, 
  ...
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
eth_cox_pfs <- tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariate**", "**Multivariate**"))
eth_cox_pfs
```
</div>
</div>
...
CH in not a factor in PFS as we observed a slight dicrease of the HR of progression for patients with no CH
```{r, fig.height = 7}
mysurv <- Surv(time = ethnicity_surv$month_at_progression_drug, event = ethnicity_surv$drug_progression_event)
...
Age at diagnosis ISS and receiving HCT are factor associated with OS in the multivariate analysis.
In the multivariate analysis, Gender is associated to better progression free survival.
...
tbl2 <- coxph(Surv(time = race_surv$month_at_os, 
  ...
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
race_cox_os <- tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariate**", "**Multivariate**"))
race_cox_os
```
</div>
...
tbl2 <- coxph(Surv(time = race_surv$month_at_progression_drug, 
  ...
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
race_cox_pfs <- tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariate**", "**Multivariate**"))
race_cox_pfs
```
</div>
</div>
...
tbl2 <- coxph(Surv(time = race_surv$month_at_os, 
  ...
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
racech_cox_os <- tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariate**", "**Multivariate**"))
racech_cox_os
```
