...
germ_data <- germline_patient_data %>% 
  ...
  select(Age_at_MMonly_diagnosis, Gender, raceeth, Whole, ISS_at_MMdx, ISSdx_grp) %>%
...
tbl1 <- raceeth_surv %>% select(raceeth, Age_at_MMonly_diagnosis, Gender, ISSdx_grp, HCT_ever) %>% 
...
tbl2 <- coxph(Surv(time = raceeth_surv$month_at_os, 
             event = raceeth_surv$os_event) ~ raceeth + Age_at_MMonly_diagnosis + Gender + ISSdx_grp + HCT_ever, data =  raceeth_surv) %>%
...
tbl2 <- coxph(Surv(time = raceeth_surv$month_at_progression_drug, 
             event = raceeth_surv$drug_progression_event) ~ raceeth + Age_at_MMonly_diagnosis + Gender + ISSdx_grp + HCT_ever, data =  raceeth_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
# eth_cox_pfs <- 
  tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**"))
# eth_cox_pfs
...
```{r}
tbl1 <- raceeth_surv %>% select(raceeth, Age_at_MMonly_diagnosis, Gender, ISSdx_grp, HCT_ever, CH_status) %>% 
  tbl_uvregression(method = survival::coxph, 
                   y = (Surv(time = raceeth_surv$month_at_progression_drug, 
                             event = raceeth_surv$drug_progression_event)),
                   exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent() %>% 
  bold_labels() %>% italicize_levels()
tbl2 <- coxph(Surv(time = raceeth_surv$month_at_progression_drug, 
             event = raceeth_surv$drug_progression_event) ~ raceeth + Age_at_MMonly_diagnosis + Gender + ISSdx_grp + HCT_ever + CH_status, data =  raceeth_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**"))
```
...
tbl1 <- raceeth_surv %>% select(raceeth, Age_at_MMonly_diagnosis, Gender, ISSdx_grp, HCT_ever, CH_status) %>% 
  ...
  bold_labels() %>% italicize_levels()
tbl2 <- coxph(Surv(time = raceeth_surv$month_at_os, 
                   event = raceeth_surv$os_event) ~ raceeth + Age_at_MMonly_diagnosis + Gender + ISSdx_grp + HCT_ever + CH_status, data =  raceeth_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**"))
```
<br>
...
tbl1 <- nhis_surv %>% select(CH_status, Age_at_MMonly_diagnosis, Gender, ISSdx_grp, Drugs_ever, HCT_ever) %>% 
  tbl_uvregression(method = survival::coxph, 
...
tbl2 <- coxph(Surv(time = nhis_surv$month_at_os, 
             event = nhis_surv$os_event) ~ CH_status + Age_at_MMonly_diagnosis + Gender + ISSdx_grp + Drugs_ever + HCT_ever, data =  nhis_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
...
tbl1 <- nhis_surv %>% select(CH_status, Age_at_MMonly_diagnosis, Gender, ISSdx_grp, HCT_ever) %>% 
  tbl_uvregression(method = survival::coxph, 
...
tbl2 <- coxph(Surv(time = nhis_surv$month_at_progression_drug, 
             event = nhis_surv$drug_progression_event) ~ CH_status + Age_at_MMonly_diagnosis + Gender + ISSdx_grp + HCT_ever, data =  nhis_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
...
tbl1 <- bl_surv %>% select(CH_status, Age_at_MMonly_diagnosis, Gender, ISSdx_grp, Drugs_ever, HCT_ever) %>% 
  tbl_uvregression(method = survival::coxph, 
...
tbl2 <- coxph(Surv(time = bl_surv$month_at_os, 
             event = bl_surv$os_event) ~ CH_status + Age_at_MMonly_diagnosis + Gender + ISSdx_grp + Drugs_ever + HCT_ever, data =  bl_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
eth_cox_os <- tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**"))
eth_cox_os
```
...
tbl1 <- bl_surv %>% select(CH_status, Age_at_MMonly_diagnosis, Gender, ISSdx_grp, HCT_ever) %>% 
  tbl_uvregression(method = survival::coxph, 
...
tbl2 <- coxph(Surv(time = bl_surv$month_at_progression_drug, 
             event = bl_surv$drug_progression_event) ~ CH_status + Age_at_MMonly_diagnosis + Gender + ISSdx_grp + HCT_ever, data =  bl_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
eth_cox_pfs <- tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**"))
eth_cox_pfs
```
...
tbl1 <- his_surv %>% select(CH_status, Age_at_MMonly_diagnosis, Gender, ISSdx_grp, Drugs_ever, HCT_ever) %>% 
  tbl_uvregression(method = survival::coxph, 
...
tbl2 <- coxph(Surv(time = his_surv$month_at_os, 
             event = his_surv$os_event) ~ CH_status + Age_at_MMonly_diagnosis + Gender + ISSdx_grp + Drugs_ever + HCT_ever, data =  his_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
eth_cox_os <- tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**"))
eth_cox_os
```
...
tbl1 <- his_surv %>% select(CH_status, Age_at_MMonly_diagnosis, Gender, ISSdx_grp, HCT_ever) %>% 
  tbl_uvregression(method = survival::coxph, 
...
tbl2 <- coxph(Surv(time = his_surv$month_at_progression_drug, 
             event = his_surv$drug_progression_event) ~ CH_status + Age_at_MMonly_diagnosis + Gender + ISSdx_grp + HCT_ever, data =  his_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
eth_cox_pfs <- tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**"))
eth_cox_pfs
```
