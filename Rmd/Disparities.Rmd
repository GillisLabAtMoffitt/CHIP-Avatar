...
tbl2 <- coxph(Surv(time = nhis_surv$month_at_os, 
             event = nhis_surv$os_event) ~ CH_status + Age_at_MMonly_diagnosis + Sex + ISSdx_grp + HCT_ever, data =  nhis_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
...
tbl2 <- coxph(Surv(time = bl_surv$month_at_os, 
             event = bl_surv$os_event) ~ CH_status + Age_at_MMonly_diagnosis + Sex + ISSdx_grp + HCT_ever, data =  bl_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
eth_cox_os <- tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**"))
eth_cox_os
```
...
tbl2 <- coxph(Surv(time = his_surv$month_at_os, 
             event = his_surv$os_event) ~ CH_status + Age_at_MMonly_diagnosis + Sex + ISSdx_grp + HCT_ever, data =  his_surv) %>%
  tbl_regression(exponentiate = TRUE) %>% bold_p(t = .05)
eth_cox_os <- tbl_merge(list(tbl1, tbl2), tab_spanner = c("**Univariable**", "**Multivariable**"))
eth_cox_os
```
...
df <- tumor_mutation_avatar %>% 
  ...
  select("KRAS", "NRAS", "TP53", "DIS3", "BRAF", "LRP1B", "FAM46C",
         "TRAF3", "CSMD3", "SP140", "CYLD", "DNAH5", BRAFv600, "RYR1", 
         "ATM", "MAX", 
...
gene_list <- c("KRAS", "NRAS", "TP53", "DIS3", "BRAF",
...
col = c("yellow", "chartreuse3", "steelblue2", "blue", "darkviolet", "cyan", "darkorchid", "darkblue", "red"
        )
...
# tiff(paste0(path, "/Disparities manuscript/oncoplot 4 by raceeth.tiff"))
