...
Seq <-
  ...
  select(c("SLID_germline", "SLID_tumor" , 
           "moffitt_sample_id_tumor", "moffitt_sample_id_germline",
           "ClinicalSpecimenLinkage_WES.Batch", # remove "BaitSet" because causes duplicates
           "moffitt_sample_id", "subject"))
...
Seq_WES_R <- 
  ...
  select(c("SLID_germline", "SLID_tumor" , 
           "moffitt_sample_id_tumor", "moffitt_sample_id_germline",
           "ClinicalSpecimenLinkage_WESBatch", # remove "BaitSet" because causes duplicates
           "moffitt_sample_id", "subject"))#,
           #"SpecimenDetail_DiseaseType", "collectiondt_tumor", "collectiondt_germline"))
colnames(Seq_WES_R)[which(names(Seq_WES_R) == "ClinicalSpecimenLinkage_WESBatch")] <-
  "ClinicalSpecimenLinkage_WES.Batch"
print(paste("We have", length(Seq_WES_Raghu$subject) ,"subject-id in Seq_WES_R with", 
            length(unique(Seq_WES_Raghu$subject)) ,"unique id"))
unique(Seq_WES_R$subject)
