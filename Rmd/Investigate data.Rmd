...
germline_patient_data %>% 
  ...
  select(Age_at_diagnosis_closest_germline, Age_at_MMonly_diagnosis, 
         time_to_drug, Gender, Race, Ethnicity, Whole, ISS, CH_status) %>%
...
germline_patient_data %>% 
  ...
  select(Age_at_diagnosis_closest_germline, Age_at_MMonly_diagnosis, diagnosis_MM_year,
         time_to_drug, Drugs_ever, HCT_ever, Gender, Race, Ethnicity, ISS, CH_status) %>%
...
p <- germline_patient_data %>% 
  ...
  geom_boxplot(
    # color= c(rep(c("#A92E5EFF", "#E65D2FFF"),3))
    ) +
  theme_minimal(base_size = 15) +
  labs(x=NULL, y="Age at Diagnosis", title="Age of diagnosis closest to germline repartition")
p + geom_jitter(shape=16, position=position_jitter(0.2)) +
...
germline_patient_data %>% 
  ...
  select(Age_at_MMonly_diagnosis, diagnosis_MM_year, MM_age_cat3, MM_age_cat5,
         time_to_drug, Drugs_ever, HCT_ever, Gender, Race1, Ethnicity, ISS, CH_status) %>%
...
germline_patient_data %>%
  distinct(avatar_id, .keep_all = TRUE) %>% 
  select(Age_at_diagnosis_closest_germline, time_to_drug, Gender, Race, Ethnicity, CH_status, ISS, ISS_grp) %>% 
